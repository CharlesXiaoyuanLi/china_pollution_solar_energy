...
### Read in data
pro_eneloss_ratio_ann = np.loadtxt("./prov_annual_eleloss_L.txt")
...
cb = mpl.colorbar.ColorbarBase(axcb, cmap=cmap, norm=norm, boundaries=bounds,
        orientation='vertical', ticks=np.linspace(0,5,11))

cb.set_label('TWh')
cb.set_ticklabels(np.arange(0,-5.5,-0.5), update_ticks=True)

plt.savefig('prov_annual_eleloss_L.ps',format='ps')
