...
### Read in data
pro_eneloss_ratio_ann = np.loadtxt("./prov_worst_day_eleloss_L.txt")


for i in range(len(pro_nm)-1):
    color[pro_nm[i]] = -1.*pro_eneloss_ratio_ann[i]
...
for key, value in patches.iteritems():
    ...
    ax.add_collection(PatchCollection(value,
        ...
        cmap=cm.GnBu, edgecolor='k', linewidth=1., zorder=2))
...
cb = mpl.colorbar.ColorbarBase(axcb, cmap=cmap, norm=norm, boundaries=bounds,
        orientation='vertical', ticks=np.linspace(0,50,11))

cb.set_label('GWh')
cb.set_ticklabels(np.arange(0,-55,-5), update_ticks=True)

plt.savefig('prov_worst_day_eleloss_L.ps',format='ps')
