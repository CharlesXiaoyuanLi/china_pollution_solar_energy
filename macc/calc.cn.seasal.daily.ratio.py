...
cn_pwden_djf = np.zeros(ndjf,dtype=float)
cn_pwden_mam = np.zeros(nmam,dtype=float)
cn_pwden_jja = np.zeros(njja,dtype=float)
cn_pwden_son = np.zeros(nson,dtype=float)
...
# Calculate density
    ...
    for i in np.arange(nprov):  #loop through provinces
        ...
        pro_eneden[:,i] = pro_power[:,i] * 24 / pro_area[i]    # [kW h / m^2]
        
    cn_powerden = np.average(pro_powerden,axis=1,weights=pro_area)

# Calculate ratio
    cn_clr_sky_pwden_daily_clim_leap = np.average(prov_clr_sky_pwden_daily_clim_leap,axis=1,weights=pro_area)
    cn_clr_sky_pwden_daily_clim_comm = np.average(prov_clr_sky_pwden_daily_clim_comm,axis=1,weights=pro_area)
    if year%4 == 0:
        pro_powerden_ratio = pro_powerden / prov_clr_sky_pwden_daily_clim_leap
        cn_powerden_ratio = cn_powerden / cn_clr_sky_pwden_daily_clim_leap
    else:
        pro_powerden_ratio = pro_powerden / prov_clr_sky_pwden_daily_clim_comm
        cn_powerden_ratio = cn_powerden / cn_clr_sky_pwden_daily_clim_comm

    print pro_powerden_ratio.shape
    print pro_powerden.T.shape

    pos_prov = 0
# write to large array
    # DJF
    prov_pwden_djf[pos_djf:pos_djf+31,:] = pro_powerden_ratio[-31:,:]
    cn_pwden_djf[pos_djf:pos_djf+31] = cn_powerden_ratio[-31:]
    pos_djf += 31
    if year%4 == 0: 
        prov_pwden_djf[pos_djf:pos_djf+31+29,:] = pro_powerden_ratio[pos_prov:31+29,:]
        cn_pwden_djf[pos_djf:pos_djf+31+29] = cn_powerden_ratio[pos_prov:31+29]
        dim_djf[pos_djf-31:pos_djf+31+29] = year*10000+djf_array_leap
        pos_djf += 31+29
        pos_prov += 31+29
    else:
        prov_pwden_djf[pos_djf:pos_djf+31+28,:] = pro_powerden_ratio[pos_prov:31+28,:]
        cn_pwden_djf[pos_djf:pos_djf+31+28] = cn_powerden_ratio[pos_prov:31+28]
        dim_djf[pos_djf-31:pos_djf+31+28] = year*10000+djf_array_comm
        pos_djf += 31+28
        pos_prov += 31+28
    # MAM
    prov_pwden_mam[pos_mam:pos_mam+92,:] = pro_powerden_ratio[pos_prov:pos_prov+92,:]
    cn_pwden_mam[pos_mam:pos_mam+92] = cn_powerden_ratio[pos_prov:pos_prov+92]
    dim_mam[pos_mam:pos_mam+92] = year*10000+mam_array
    pos_mam += 92
    pos_prov += 92
    # JJA
    prov_pwden_jja[pos_jja:pos_jja+92,:] = pro_powerden_ratio[pos_prov:pos_prov+92,:]
    cn_pwden_jja[pos_jja:pos_jja+92] = cn_powerden_ratio[pos_prov:pos_prov+92]
    dim_jja[pos_jja:pos_jja+92] = year*10000+jja_array
    pos_jja += 92
    pos_prov += 92
    # SON
    prov_pwden_son[pos_son:pos_son+91,:] = pro_powerden_ratio[pos_prov:pos_prov+91,:]
    cn_pwden_son[pos_son:pos_son+91] = cn_powerden_ratio[pos_prov:pos_prov+91]
...
for i in range(nprov):
    print pro_nm[i]
    print prov_clr_sky_pwden_mon_clim[i,:]

### Write the data to ncfile
outfile = Dataset('prov.seasonal.daily.powerden.ratio.nc','w',format='NETCDF4')
...
prov_pwden_djf_out = outfile.createVariable('prov_pwden_ratio_djf',np.float32,('province','time_djf'))
prov_pwden_mam_out = outfile.createVariable('prov_pwden_ratio_mam',np.float32,('province','time_mam'))
prov_pwden_jja_out = outfile.createVariable('prov_pwden_ratio_jja',np.float32,('province','time_jja'))
prov_pwden_son_out = outfile.createVariable('prov_pwden_ratio_son',np.float32,('province','time_son'))

cn_pwden_djf_out = outfile.createVariable('cn_pwden_ratio_djf',np.float32,('time_djf',))
cn_pwden_mam_out = outfile.createVariable('cn_pwden_ratio_mam',np.float32,('time_mam',))
cn_pwden_jja_out = outfile.createVariable('cn_pwden_ratio_jja',np.float32,('time_jja',))
cn_pwden_son_out = outfile.createVariable('cn_pwden_ratio_son',np.float32,('time_son',))
...
prov_pwden_djf_out[:] = np.transpose(prov_pwden_djf)*100
prov_pwden_djf_out.units = '%'
prov_pwden_mam_out[:] = np.transpose(prov_pwden_mam)*100
prov_pwden_mam_out.units = '%'
prov_pwden_jja_out[:] = np.transpose(prov_pwden_jja)*100
prov_pwden_jja_out.units = '%'
prov_pwden_son_out[:] = np.transpose(prov_pwden_son)*100
prov_pwden_son_out.units = '%'

cn_pwden_djf_out[:] = np.transpose(cn_pwden_djf)*100
cn_pwden_djf_out.units = '%'
cn_pwden_mam_out[:] = np.transpose(cn_pwden_mam)*100
cn_pwden_mam_out.units = '%'
cn_pwden_jja_out[:] = np.transpose(cn_pwden_jja)*100
cn_pwden_jja_out.units = '%'
cn_pwden_son_out[:] = np.transpose(cn_pwden_son)*100
cn_pwden_son_out.units = '%'
